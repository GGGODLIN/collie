...
| ✓ | 換 Claude 帳號重開 session | U 帳號用量 | 2026-09-26 已部署，手機操作待驗收 | — |
| ✓ | 從列表直接批准 | 1-1 批准 | 2026-09-26 已部署，手機操作待驗收 | — |
| ✓ | 手機上的「白話／跟丟了」 | 使用者 2026-09-24 觀察 | 2026-09-26 已部署，手機操作待驗收 | — |
...
**狀態**（2026-09-26）：已部署。pane 選單「切換帳號」讀 `accounts.toml`（Team-P、Team-S），在原 pane 結束 Claude、確認退出後以 `--resume` 接續。實測（拋棄式 session、真 Team-P／Team-S）：閒置、輸入框有草稿、工作中三種都只剩一個程序、帳號正確、Opus 1M 與 high 保留、草稿與 `/exit` 沒被送進對話。做法草圖的 `--model`／`--effort` 沒採用：實測 `--resume` 會自己還原最後一輪的設定，而 transcript 記不到 `[1m]`。未實測：權限對話框（blocked）狀態下切換、`/effort` 後拒絕切換的真機路徑。
...
**狀態**（2026-09-26）：[實作 commit](https://github.com/GGGODLIN/collie/commit/dbe4d3dcce915a3b491738da1539a333dcf5f3fe) 已部署到本機 Collie；局部測試、空畫面收起的獨立複驗、lint、兩端型別與 build 通過，手機網址送出的版本與部署檔一致。手機實際操作待使用者驗收；本次數字 React Doctor score 對帳經使用者豁免，未為補分數開啟診斷外送。
...
**狀態**（2026-09-26）：採方向 a 已部署。pane 選單加「白話」「跟丟了」，結果開在同畫面的閱讀彈窗；主機依 `retell.toml` 跑一次 sidecar `ww --session-id <id> --json --source http`，prompt 與快取沿用 sidecar（[ADR 0074](/.adr/0074-retell-is-a-one-shot-operator-child.md)）。實測兩種模式各產生一次（41 秒、79 秒），第二次命中快取。
