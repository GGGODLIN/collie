...
- **A Muse reply that quotes a dialog no longer blocks every send until the transcript moves.** When Muse printed the text of an approval, a question or a review screen as its last reply, the reply check took the quote for a live dialog and refused each send from the app, with no way out but "Type into terminal". A live Muse dialog owns the keyboard, so its input box is always bare; the same rows above a box holding a draft or a placeholder are transcript. The check now refuses a question, checkbox or note match only above a bare box, and a review match only with its live header directly above, which is the same test the buttons already use. An approval or trust match still refuses on its own, since neither leaves a live box. Thanks @jpcarranza94 (#262, fixes #260).
